...
namespace BlackCat\Database\Packages\PrivacyRequests\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\PrivacyRequests\Dto\PrivacyRequestDto;
use BlackCat\Database\Packages\PrivacyRequests\Mapper\PrivacyRequestDtoMapper;
 ...
 */
final class PrivacyRequestsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private PrivacyRequestRepository $privacyRequestRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'privacy_requests' => $this->privacyRequestRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\PrivacyRequests\ModuleException("Record {$idStr} not found for locking.");
            }
