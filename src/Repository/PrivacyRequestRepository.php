...
use BlackCat\Database\Packages\PrivacyRequests\Repository\PrivacyRequestRepositoryInterface;
...
class PrivacyRequestRepository implements PrivacyRequestRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\PrivacyRequests\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'status', 'processed_at', 'meta' ]);
          return count($rows);
        }
