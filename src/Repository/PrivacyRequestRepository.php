...
namespace BlackCat\Database\Packages\PrivacyRequests\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\PrivacyRequests\Definitions;
use BlackCat\Database\Packages\PrivacyRequests\Criteria;
use BlackCat\Database\Packages\PrivacyRequests\Dto\PrivacyRequestDto as Dto;
use BlackCat\Database\Packages\PrivacyRequests\Mapper\PrivacyRequestDtoMapper as RowMapper;
...
final class PrivacyRequestRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\PrivacyRequests\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'status', 'processed_at', 'meta' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\PrivacyRequests\Definitions::table());
            return $bulk->upsertMany($rows, [ 'id' ], [ 'status', 'processed_at', 'meta' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\PrivacyRequests\Dto\PrivacyRequestDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\PrivacyRequests\Dto\PrivacyRequestDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\PrivacyRequests\Mapper\PrivacyRequestDtoMapper::fromRow($row);
    }
